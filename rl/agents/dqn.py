...
class DQNAgent(AbstractDQNAgent):
    """Write me
    """
    def __init__(self, model, target_model, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
...
class DistributionalDQNAgent(AbstractDQNAgent):
    """For our taxi / maze environment.
    ...
    def __init__(self, model, target_model,  num_atoms, v_min, v_max, policy=None, test_policy=None, enable_double_dqn=True, enable_dueling_network=False,
        ...
        # Validate (important) input.
        if hasattr(model.output, '__len__') and len(model.output) <= 1:
            raise ValueError('Model "{}" has one output. Distributional DQN expects a model that has a multiple outputs (one for each action).'.format(model))

        if model.output[0]._keras_shape != (None, num_atoms):
            raise ValueError("Model output has invalid shape. Distributional DQN expects a model that has {} atoms for each action.".format(num_atoms))
    ...
    def backward(self, reward, terminal, env_number):
        ...
        if self.step > self.nb_steps_warmup and self.step % self.train_interval == 0:
            ...
            m_prob = [np.zeros((self.batch_size, self.num_atoms)) for i in range(self.nb_actions)]
            ...
            q = np.sum(np.multiply(z_concat, np.array(self.z)), axis=1) #(num_atoms x nb_actions?)
            q = q.reshape((self.batch_size, self.nb_actions))
            actions = np.argmax(q, axis=1)

            # Project Next State Value Distribution (of optimal action) to Current State
            for i in range(self.batch_size):
                if terminal1_batch[i]: # Terminal State
                    ...
                    for j in range(self.num_atoms):
                        ...
                        #print self.v_min, self.v_max, self.delta_z
                        #print reward_batch
                        #print reward_batch[i], self.gamma, self.z[j]
                        #print Tz, bj
                        #print i, j
                        #print action_batch[i]
                        #print int(m_l), int(m_u)
                        #print actions[i], "\n"
    ...
    def metrics_names(self):
        ...
        return names
        '''
        #assert len(self.model.output_names) == 2
        #dummy_output_name = self.model.output_names[1]
        #print self.model.metrics_names
        #model_metrics = [name for idx, name in enumerate(self.model.metrics_names) if idx not in (1, 2)]
        #model_metrics = [name.replace(dummy_output_name + '_', '') for name in model_metrics]
        #print self.policy.metrics_names

        #Loss is metric 0.
        names = self.model.metrics_names+ self.policy.metrics_names[:]
        if self.processor is not None:
            names += self.processor.metrics_names[:]
        return names
    ...
    def compute_batch_q_values(self, state_batch):
        ...
        assert len(q_values) == self.nb_actions
        assert q_values[0].shape == (len(state_batch), self.num_atoms)
        return q_values

    def compute_q_values(self, state):
        z = self.compute_batch_q_values([state])#Should be giving a list. Shape = (nb_actions, num_atoms)
        z_concat = np.vstack(z)
